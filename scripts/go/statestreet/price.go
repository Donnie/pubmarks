...
// applyClosingPriceExtras sets base_currency and price (float64) from the Fund Market
// Price "Closing Price" string, e.g. "$645.09" or "USD 12.34".
func applyClosingPriceExtras(out map[string]any) {
	raw, ok := out["closing_price"].(string)
	if !ok {
		return
	}
